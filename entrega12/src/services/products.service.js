...
const ProductsDao= configs.persistence === "FS" ? new Products(`${__dirname}/dao/fileManagers/files/products.json`) : new Products();
...
const getProductsService= async(query, queryValue, limitNumber,pageNumber, sortValue)=>{
    const products= configs.persistence === "MONGO" ? await manager.getRepository(query, queryValue, limitNumber,pageNumber, sortValue) : await manager.getRepository();
    return products;
};
