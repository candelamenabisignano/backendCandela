...
const getProductsView= async(req,res)=>{
    ...
    try {
        if(configs.persistence === 'MONGO'){
            ...
            res.render('products', {persistence:configs.persistence,products: docs, hasPrevPage, hasNextPage, nextPage, prevPage, nextLink, prevLink, user:req.user}); 
        }else{
            const products= await getProductsService();
            res.render('products', {persistence:configs.persistence === 'MONGO' ? true : false,user:req.user, products:products})
        }
    } catch (error) {
